...
/**
 * macOS usbmuxd listens on a unix socket at /var/run/usbmuxd
 * Windows usbmuxd listens on port 27015
 */
const DEFAULT_USBMUXD_SOCKET = process.platform === `win32` ? { port: 27015 } : { path: `/var/run/usbmuxd` };
 ...
 *
 * @param {?string} socketPath The path to the usbmuxd port or unix socket
...
async function getDefaultSocket (socketPath = DEFAULT_USBMUXD_SOCKET, timeout = 5000) {
  // Only validate socketPath if a paths is provided (non-windows)
  if (socketPath.path && !await fs.exists(socketPath)) {
